...
class BinanceObClass:
    ...
    def get_orderbook(self):
        ...
        if (
            "lastUpdateId" not in response
            or "bids" not in response
            or "asks" not in response
        ):
            print(
                f"Veri hatalı, ham veriyi kontrol edin. {self.orderbookURL} {response}"
            )
